...
|:------:|:--------:|:--------:|:------------|:---------|
| EVM | ETH、BNB、MATIC、AVAX 等原生资产（用于支付 Gas） | ERC-20 | 支持任意 ERC-20 代币，适合按业务需要接入 USDT、USDC 或其他链上资产收款 | 在后台添加代币合约地址并启用对应公链 |
...
登录管理后台，进入 **系统** **系统钱包** 页面，复制系统钱包地址，并在每条启用的 EVM 链上向该地址充值少量原生资产用于支付 Gas，例如 ETH、BNB、MATIC 等。

系统钱包只用于平台基础设施交易，例如智能合约部署、智能合约归集等需要由系统主动发起的链上操作；业务收款资金仍按合约规则流向你的收款归集地址。这里不需要存入业务资金，只需要保留覆盖近期操作的小额 Gas，避免因 Gas 不足导致合约部署或归集任务无法广播。
